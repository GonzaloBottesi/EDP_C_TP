...
class Central():
    ...
    def __init__(self):
        ...
        self.phones = self.extractFile('phones.csv')
        self.callLog = dict()
        self.SMSLog = dict()
        ...
        try:
            with open(archivo_csv, mode='r', newline='', encoding = 'utf-8') as archivo:
                ...
                for telefono in lector_csv:
                    phones[telefono[0]]=Telefono(telefono[0],telefono[1],telefono[2],telefono[3],
    ...
    def handleSMS (self, packet : list):
        receptor = packet[2]
...
## Modelo de paquete a enviar: ['LLAMADA', Emisor , Receptor, datetime , pedido]. R = REQUEST ; B = BUSY ; N = NOT FOUND ; K = OK ; S = STOP ; F = RECHAZADO
    ...
    def handleCall(self, packet : list):
        """Inicia la llamada verificando la disponibilidad de ambos teléfonos."""
        
        originNumber = packet[1]
        destinyNumber = packet[2]
        
        if not (originNumber in self.phones and destinyNumber in self.phones):
            print(f"El teléfono con número {destinyNumber} no está registrado en la central.")
            packet[4] = 'N'
            return packet
        
        #Ambos estan registrados en la central
        originPhone = self.phones[originNumber]
        destinyPhone = self.phones[destinyNumber]
        
        if packet[4] == 'R':
            # Verificar si ambos teléfonos están disponibles para la llamada
            if not (self.verifyWeb(originNumber) and self.verifyWeb(destinyNumber)):
